...
; Setup debug mode
(eval-and-compile
  (defvar barista-debug-mode
    (or (getenv "DEBUG") init-file-debug)
    "Debug mode, enable through DEBUG=1 or use --debug-init.")
  (setq debug-on-error (and (not noninteractive) barista-debug-mode)
        jka-compr-verbose barista-debug-mode))

(defvar barista-verbose-byte-compile-warnings nil)

;; Disable certain byte compiler warnings to cut down on the noise.
(if (or barista-debug-mode barista-verbose-byte-compile-warnings)
...
;; Ensure `barista' is in `load-path'
...
;; Require necessary barista things
(require 'barista-core)
...
(eval-and-compile
  ...
  (defvar barista-root (file-truename user-emacs-directory)
    "Root of the barista.")

  (defvar barista-dir (concat barista-root "core/")
    "The main directory of barista-emacs configuration."))
...
;; Here we go
(defmacro barista/init (&rest body)
